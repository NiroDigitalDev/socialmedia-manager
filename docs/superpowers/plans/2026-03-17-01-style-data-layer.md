# Style Data Layer Implementation Plan

> **For agentic workers:** Use superpowers:subagent-driven-development or superpowers:executing-plans.

**Goal:** Create a Style tRPC router with full CRUD + preview generation + seed, and custom hooks with optimistic updates.
**Depends on:** None
**Architecture:** The Style tRPC router replaces all existing REST endpoints under `/api/styles/*` with type-safe tRPC procedures. It uses `orgProtectedProcedure` for org-scoped access, Gemini for image generation and style extraction, and `StoredImage` for binary image storage. Custom React hooks wrap each procedure with TanStack Query patterns including optimistic updates for create/delete.
**Tech Stack:** tRPC v11, `@trpc/tanstack-react-query`, TanStack Query, Prisma, Gemini (`@google/genai`), Zod, Sonner (toast)

---

## Task 1: Create the Style tRPC Router

**File:** `src/lib/trpc/routers/style.ts` (new file)
...
## Task 2: Register the Style Router

**File:** `src/lib/trpc/router.ts` — add import and register
...
## Task 3: Create Style Hooks

**File:** `src/hooks/use-styles.ts` (new file)
...
```bash
git add src/lib/trpc/routers/style.ts src/lib/trpc/router.ts src/hooks/use-styles.ts
git commit -m "feat: add style tRPC router with CRUD, preview generation, seed, and custom hooks"
```
...
1. **Image serving pattern:** Sample and reference images are stored as binary `Bytes` in the `StoredImage` model. The tRPC layer returns URL strings like `/api/images/{id}?type=stored` — the existing image serving endpoint handles binary delivery with immutable cache headers.

2. **Org scoping for list:** The `list` procedure returns both org-owned custom styles (`orgId === ctx.orgId`) and predefined styles (`isPredefined === true, orgId === null`). Predefined styles have no org association.

3. **Delete safety:** Only non-predefined styles owned by the current org can be deleted. The delete also cascades to associated `StoredImage` records.

4. **`fromImage` accepts base64:** Unlike the old REST endpoint which used `FormData`, the tRPC mutation accepts `base64` + `mimeType` directly. The calling component must convert the file to base64 before calling.

5. **`geminiText.generateContent`** returns a string directly (the wrapper in `src/lib/gemini.ts` extracts `.text`). When called with an array of parts (image + text), it works as Gemini vision.
