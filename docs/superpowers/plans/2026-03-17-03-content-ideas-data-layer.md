# Content Ideas Data Layer Implementation Plan

> **For agentic workers:** Use superpowers:subagent-driven-development or superpowers:executing-plans.

**Goal:** Add AI content idea generation procedure to the existing content tRPC router, porting the sophisticated Gemini prompt from the REST endpoint.
**Depends on:** None (the existing `contentRouter` in `src/lib/trpc/routers/content.ts` already has the other content procedures)
**Architecture:** A single `generateIdeas` mutation is added to the existing content router. It fetches the source text, builds the Gemini prompt (ported verbatim from the REST endpoint), calls Gemini text generation, parses/validates the JSON response, and batch-creates `ContentIdea` records. The hook wraps this with cache invalidation and toast feedback.
**Tech Stack:** tRPC v11, `@trpc/tanstack-react-query`, TanStack Query, Prisma, Gemini (`@google/genai`), Zod, Sonner (toast)

---

## Task 1: Add `generateIdeas` Mutation to Content Router

**File:** `src/lib/trpc/routers/content.ts` — add the mutation at the end of the router, before the closing `});`
...
## Task 2: Add `useGenerateIdeas()` Hook

**File:** `src/hooks/use-content.ts` — replace entire file
...
```bash
git add src/lib/trpc/routers/content.ts src/hooks/use-content.ts
git commit -m "feat: add AI content idea generation to content tRPC router with full Gemini prompt"
```
...
1. **The Gemini prompt is the product's core IP.** It is ~2000 words of carefully crafted prompt engineering including quality filters (Expert Test, Screenshot Test, Specificity Test, So What? Test), content patterns, bad-vs-good examples, content type guidelines, and detailed slide prompt requirements. It has been ported **verbatim** from `src/app/api/content/ideas/generate/route.ts`.

2. **Slide prompt rules are critical.** The prompt explicitly instructs Gemini to produce STYLE-AGNOSTIC and COLOR-AGNOSTIC slide prompts. This is because visual styles and brand colors are applied by separate systems at generation time. The slide prompts describe only content layout, text, and composition.

3. **Org scoping:** The mutation sets `orgId: ctx.orgId` on every created `ContentIdea`. The old REST endpoint did not set orgId (it was nullable). This is an improvement for multi-tenant correctness.

4. **Project scoping:** The optional `projectId` is passed through to each `ContentIdea` record, enabling project-level content filtering.

5. **Slide count validation:** The mutation normalizes `slideCount` (min 2, max 10 for carousel; always 1 for static) and pads/trims `slidePrompts` to match. This prevents malformed AI output from creating inconsistent records.

6. **Response format:** The mutation returns `{ count: number }` rather than the full array of ideas. This is intentional — the UI should invalidate the `listIdeas` query to fetch fresh data rather than manually inserting into the cache, because the ideas have server-generated IDs and timestamps.

7. **Existing hooks updated:** The `useDeleteSource`, `useCreateSource`, `useToggleIdeaSave`, and `useBulkDeleteIdeas` hooks have been improved with targeted `queryKey`-based invalidation (instead of the previous broad `queryClient.invalidateQueries()` without a key) and `onError` toasts.
